...
export function JDListPage() {
  ...
  const evalCountByJD = evaluations.reduce<Record<string, number>>(
    (acc, ev) => {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-sm">
            {jds.length} job descriptions total
          </p>
        </div>
        <Link to="/jd/new">
          <Button>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center flex-wrap gap-1.5 mt-3">
                        ...
                        {jd.department && (
                          <span className="inline-flex items-center text-xs font-medium bg-gray-100 text-gray-600 rounded-full px-2.5 py-1">
                            {jd.department.name}
                          </span>
                        )}
                        {jd.experienceLevel && (
                        ...
                        {jd.employmentTypes &&
                          jd.employmentTypes.map((et) => (
                            <span
                              key={et}
                              className="inline-flex items-center text-xs font-medium bg-emerald-50 text-emerald-700 rounded-full px-2.5 py-1"
                            >
                              {et.replace("_", " ")}
                            </span>
                          ))}
                        {jd.extractedSkills &&
                          jd.extractedSkills.length > 0 && (
                            <>
                              {jd.extractedSkills.slice(0, 4).map((skill) => (
                                <Badge key={skill} variant="info">
                                  {skill}
                                </Badge>
                              ))}
                              {jd.extractedSkills.length > 4 && (
                                <Badge variant="default">
                                  +{jd.extractedSkills.length - 4} more
                                </Badge>
                              )}
                            </>
                          )}
