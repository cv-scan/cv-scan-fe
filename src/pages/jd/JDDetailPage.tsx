...
const CLASSIFICATION_CARD_STYLES: Record<Classification, string> = {
  PASS: "text-green-600",
...
export function JDDetailPage() {
  ...
  const classificationCounts = stats?.classificationBreakdown ?? {};
  const passCount = classificationCounts.PASS ?? 0;
  const waitlistCount = classificationCounts.WAITLIST ?? 0;
  const failCount = classificationCounts.FAIL ?? 0;
  const classificationOrder: Classification[] = ["PASS", "WAITLIST", "FAIL"];
    ...
    <div className="max-w-3xl space-y-5">
      ...
      {activeTab === "overview" && (
        <>
          {(jd.department ||
            jd.location ||
            (jd.employmentTypes && jd.employmentTypes.length > 0) ||
            jd.experienceLevel) && (
            <Card>
              ...
              <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                ...
                {jd.location && (
                  <>
                    ...
                    <dd className="text-gray-900 font-medium">{jd.location}</dd>
                  </>
                )}
                {jd.experienceLevel && (
                  <>
                ...
                {jd.employmentTypes && jd.employmentTypes.length > 0 && (
                  <>
                    <dt className="text-gray-500">Employment Types</dt>
                    <dd className="flex flex-wrap gap-1.5">
                      {jd.employmentTypes.map((et) => (
                        <span
                          key={et}
          ...
          {statsLoading ? (
            ...
            <Card className="text-center py-16">
              ...
              <p className="text-gray-500 text-sm mb-4">
                No evaluations yet for this JD.
              </p>
              <Button
                size="sm"
                onClick={() => {
            ...
            <div className="space-y-5">
                  ...
                  return (
                    <Card key={classification}>
                      ...
                      <p
                        className={cn(
                          "text-3xl font-bold",
                          CLASSIFICATION_CARD_STYLES[classification],
                        )}
                      >
                        {value}
                      </p>
              ...
              {Object.keys(stats.recommendationBreakdown).length > 0 && (
                <Card>
                  ...
                  <div className="space-y-3">
                    {(
                      Object.entries(stats.recommendationBreakdown) as [
                        Recommendation,
                        number,
                      ][]
                    ).map(([rec, count]) => {
                        ...
                        <div key={rec}>
                          ...
                          <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                            <div
                              className={cn(
                                "h-full rounded-full",
                                RECOMMENDATION_COLORS[rec] ?? "bg-gray-400",
                              )}
                              style={{ width: `${pct}%` }}
                            />
                          </div>
              ...
              {Object.keys(stats.categoryAverages).length > 0 && (
                <Card>
                  ...
                  <div className="space-y-3">
                    {(
                      Object.entries(stats.categoryAverages) as [
                        ScoreCategory,
                        number,
                      ][]
                    ).map(([cat, score]) => (
