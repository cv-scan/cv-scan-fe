    ...
    <div className="max-w-3xl space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-900 truncate">{cv.fileName}</h1>
        </div>
        <div className="flex items-center gap-3">
      ...
      {(cv.status === 'pending' || cv.status === 'processing') && (
        <Card className="flex items-center gap-3 bg-amber-50 border-amber-200" padding={false}>
          <div className="p-4 flex items-center gap-3">
            <Spinner size="sm" className="text-amber-500" />
            <div>
              <p className="text-sm font-medium text-amber-800">Processing your CV</p>
              <p className="text-xs text-amber-600 mt-0.5">This may take a moment. Page will auto-refresh.</p>
            </div>
          </div>
        </Card>
      )}
      ...
      {parsed && (
        <>
              ...
              <dl className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {parsed.name && (
                  <div>
                    ...
                    <dd className="text-sm font-semibold text-gray-900">{parsed.name}</dd>
                  </div>
                )}
                {parsed.email && (
                  <div>
          ...
          {parsed.experience && parsed.experience.length > 0 && (
            <Card>
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-4">Experience</h2>
          ...
          {parsed.education && parsed.education.length > 0 && (
            <Card>
              <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-4">Education</h2>
