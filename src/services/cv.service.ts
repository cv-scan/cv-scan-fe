...
function normalize(raw: any): CV {
  const rawStatus = (raw.parseStatus as string | undefined)?.toLowerCase() ?? ''
  const status: CVStatus = rawStatus === 'completed' ? 'parsed' : rawStatus as CVStatus

  return {
    id: raw.id,
    fileName: raw.fileName,
    status,
    parsedData: (raw.candidateName || raw.candidateEmail)
      ? { name: raw.candidateName ?? undefined, email: raw.candidateEmail ?? undefined }
      : undefined,
